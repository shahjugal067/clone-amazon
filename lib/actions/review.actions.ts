...
import { IReviewDatails, IReviewInput } from "@/types"
...
export async function createUpdateReview({
    ...
    try {
        ...
        const review = await ReviewInputSchema.parse({
            ...data,
...
const updateProductReview = async (productId: string) => {
     ...
     //calculate the total no. of review and average rating
  ...
  const reviews = await Review.find({ product: productId })
    ...
    return {
        data: JSON.parse(JSON.stringify(reviews)) as IReviewDatails[],
