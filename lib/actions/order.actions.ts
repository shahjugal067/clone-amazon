...
export const createOrder = async(clientSideCart: Cart) =>{
    try {
        ...
        if(!session) throw new Error('User not authorized')
            ...
            //recalculate price and delivey dat on the server
...
export async function approvePayPalOrder(
    ...
    try {
        ...
        if(!captureData || captureData.id !== order.paymentResult?.id || 
...
export const calcDeliveryDateAndPrice = async({
    ...
    const itemsPrice = round2(
        ...
        const taxPrice = !shippingAddress ? undefined : round2(itemsPrice * 0.13)
