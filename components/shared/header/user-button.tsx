            ...
            <DropdownMenu>
                ...
                {session ? (
                    <DropdownMenuContent className=" w-56" align="end" forceMount>
                        ...
                        <DropdownMenuGroup>
                            <Link className=" w-full hover:bg-gray-500" href='/account'>
                            <DropdownMenuItem>Your account</DropdownMenuItem>
                            </Link>
                            <Link className="w-full hover:bg-gray-500" href='/account/orders'>
                            <DropdownMenuItem>Your Orders</DropdownMenuItem>
                            </Link>
                            {session.user.role === 'Admin' && (
                                <Link className="w-full hover:bg-gray-500" href={'/admin/overview'}>
                                    <DropdownMenuItem>Admin</DropdownMenuItem>
                                </Link>
                            )}
                        </DropdownMenuGroup>
                        <DropdownMenuItem className="p-0 mb-1">
                            <form action={SignOut} className="w-full hover:bg-gray-500">
                                <Button className="w-full py-4 px-2 h-4 justify-start hover:bg-green-500" variant='ghost'>
                                    Sign Out
                                </Button>
                            </form>
