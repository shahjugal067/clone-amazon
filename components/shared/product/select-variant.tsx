        ...
        {product.colors.length > 0 && (
            <div className=" space-x-2 space-y-2">
                ...
                {product.colors.map((x:string)=>(
                    <Button asChild variant={"outline"} key={x}
                    className={selectedColor ===x ? 'border-2 border-primary':'border-2'} >

                       <Link key={x} replace scroll={false} 
                       href={`?${new URLSearchParams({color:x, size:selectedSize})}`} >
                       <div style={{backgroundColor:x}} className=" h-4 w-4 rounded-full border border-muted-foreground" >
                        </div>
                        {x}
                       </Link>
                    </Button>
