...
export default function SalesAreaChart({data} : {data:any[]}) {
    ...
    return (
        <ResponsiveContainer width ='100%' height={400} >
            ...
            <XAxis dataKey='data' tick={<CustomXAxisTick/>} interval={3} />
            <YAxis fontSize={12} tickFormatter={(value: number) => `$${value}`} />
                <Tooltip content={<CustomTooltip />} />
                <Area type={"monotone"} dataKey='totalSales' strokeWidth={2}
                stroke={STROKE_COLORS[color.name][theme || 'light']} fillOpacity={0.8}
